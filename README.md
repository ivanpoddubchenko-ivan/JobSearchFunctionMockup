...
Проєкт зараз є **фронтенд-прототипом**: вакансії — тимчасові/захардкоджені
дані без бекенду, але **автентифікація вже реальна** через Supabase.
...
- Механізм світлої/темної теми лишається в коді (`useTheme`), але перемикач
  прибрано з хедера дашборду — зараз тема завжди світла
...
- Персистентність збережених вакансій і статусу заявок на них (зараз
  скидаються при перезавантаженні — на відміну від сесії логіну)
- Прямі посилання на конкретну вакансію
- Тести
...
- Узгодити брендинг: хедер дашборду й сторінка логіну вже брендовані під
  BHMP Network, але назва проєкту в цьому README досі "Pathways"
...
- `src/components/shared.tsx` — спільні UI-атоми й CSS-var токени
  (`Logo`, `ThemeToggle`, `JobCard`, `JobDetail`)
